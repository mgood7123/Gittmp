...
int check_lg2(int error, const char *message, const char *extra)
{
...
int fatal(const char *message, const char *extra)
{
	if (extra)
		fprintf(stderr, "%s %s\n", message, extra);
	else
		...
		void * (payload))
{
	...
	if (!using_defined) {
		if (!username) {
			printf("Username: ");
			if (getline_stdin(&username) <= 0) {
				fprintf(stderr, "Unable to read username: %s\n", strerror(errno));
				return -1;
			}
		}
		pi(strlen(username))
		pc(username[strlen(username)])
		pc(username[strlen(username)-1])
		ps(username)
		if (!password) {
			printf("Password: ");
			if (getline_stdin(&password) <= 0) {
